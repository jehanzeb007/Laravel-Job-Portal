        	...
        	<div class="row">
                <div class="col-md-4 col-sm-4">
                    <div class="munal-frnd-list">
                        ...
                        {!! Form::select('type', array('employers' => "Employers",'jobseekers' => 'JobSeekers'), 'employers', array('class' => 'form-control', 'onchange' => "update_chat(this)")) !!}
                        @endif
                        @if(!empty($relatedJobs->count()))
                        	@foreach($relatedJobs as $relatedJob)
                            	<a href="javascript:void(0)" onClick="loadChat('{{$relatedJob->job_id}}','{{$relatedJob->job_applid_user}}',false)">
                                    <div class="frnd-list">
...
@section('footer')
    ...
    function update_chat(elem){
        ...
        if (chat_type == "jobseekers"){
            window.location.href = "/chat-room-seeker";
        }
    }
	...
	function loadChat(job_id,poster_id,reload_auto){
		...
		$.ajax({
			type: "get",
			url: '{{url('')}}/get-chat-employer/'+user_id+'/'+job_id+'/'+poster_id,
			data: dataString,
			cache: false,
