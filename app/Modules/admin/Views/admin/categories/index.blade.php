...
@section('title')    
    {{Config::get('constants.site.name')}} | Categories
@stop
...
<div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12">
        <div>
            ...
            <div class="pull-right"><a class="btn btn-primary btn-small primary_color grouped_elements" href="{{route('add_categorie')}}">+ Add subcategory</a></div>
            <div class="clearfix"></div>
        </div>
        <hr/>
        <div style="margin-bottom: 20px;">
            <div class="col-md-2" style="padding-left: 0px;">
                <input id="searchinput" name="name" placeholder="Search By Category Name" class="form-control input-md" type="search">
            </div>
            <div class="clearfix"></div>
        </div>
        <div class="row userInfo">
            <div class="col-xs-12 col-sm-12">
                <table class="footable footable-loaded tablet breakpoint">
                    ...
                    <tbody id="content">
                        @include('admin::admin.categories.listing', ['categorie' => $categorie])
                    </tbody>
                </table>
            </div>
        </div>
...
@section('footer_assets')
    ...
    <script>
        ...
        function click_showbtn(id) {
            var id = id;
            console.log("id",id);
            var text = $("#showbtn_"+id).text();
            console.log("text",text);
            if (text=="show subcategories") {
                console.log("show");
                $('.child_'+id).show(1000);
                $("#showbtn_"+id).text('hide subcategories');
            }else{
                $('.child_'+id).hide(1000);
                $("#showbtn_"+id).text('show subcategories');
            }
        ...
        if(loadingComplete == true){
            $(window).scroll(function() {
                if( $(window).scrollTop() + $(window).height() > $(document).height() + 510 ) {
                    if (page < last_page) {
                        page++;
                        var url = "{{URL::route('listing_categorie')}}?" + formatUrlParams();
                        fetchData(url, '0');
                    }
                }
            });
