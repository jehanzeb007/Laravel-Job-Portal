...
<div class="topBar" >
    <div class="navbar primary_color navbar-fixed-top megamenu" role="navigation" style="background:#9b59b6">
        <h1 class="section-title-inner" style="text-align:center; padding:20px;">
                <span style="color:white;">Add subcategory</span>
        </h1>
    </div>
</div>
<div class="clearfix"></div>
<div class="pop_up">
   {!! Form::open(['url' => route('store_categorie'), 'class' => 'form-horizontal','method' => 'post']) !!}
            @if ($errors->any())
    ...
    <div class="row">    
        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
            <div class="form-group {{ $errors->has('subcategorie') ? 'has-error' : ''}}"style=" margin-right: 1px;">
                ...
                {!! Form::text('subcategorie', null, ['class' => 'form-control','placeholder' => 'Subcategory']) !!}
            </div>
        </div> 
        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
            <div>
                <div class="form-group  {{ $errors->has('categorie') ? 'has-error' : ''}}"style=" margin-right: 1px;">
                    ...
                    {!! Form::select('categorie', $categorie, null, array('class' => 'form-control')) !!}
                </div>
            </div>
        </div>    
    </div>
    <div class="row"> 
        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
            <div class="form-group {{ $errors->has('icon') ? 'has-error' : ''}}"style=" margin-right: 1px;">
                ...
                {!! Form::label('*', '*', ['class' => 'text-danger']) !!}
                {!! Form::text('icon', null, ['class' => 'form-control','placeholder' => 'Icon Class']) !!}
            </div>
        </div> 
    </div>
    ...
    <div class="row">   
        <div class="form-group">
            <div class="col-lg-2 col-md-2 col-sm-4 col-xs-4" style="float:right; margin-top:85px;">
                @if( \Session::get('success_msg') )
                    {!! Form::button('Add', ['class' => 'btn btn-primary form-control primary_color auto_click','onclick'=> 'parent.closeFancyBox()']) !!}
                @else
                    {!! Form::submit('Add', ['class' => 'btn btn-primary form-control primary_color']) !!}
                @endif
            </div>
        </div>
    </div>
