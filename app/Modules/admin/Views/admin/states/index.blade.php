...
@section('title')    
    {{Config::get('constants.site.name')}} | States
@stop
...
<div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12">
        <div>
            ...
            <div class="pull-right"><a class="btn btn-primary btn-small primary_color grouped_elements" href="{{route('add_state')}}">+ Add State</a></div>
            <div class="clearfix"></div>
        </div>
        <hr/>
        <div style="margin-bottom: 20px;">
            <div class="col-md-2" style="padding-left: 0px;">
                <input id="searchinput" name="name" placeholder="Search By State Name" class="form-control input-md" type="search">
            </div>
            <div class="clearfix"></div>
        </div>
        <div class="row userInfo">
            <div class="col-xs-12 col-sm-12">
                <table class="footable footable-loaded tablet breakpoint">
                    ...
                    <tbody id="content">
                        @include('admin::admin.states.listing', ['state' => $state])
                    </tbody>
                </table>
            </div>
        </div>
        ...
        if(loadingComplete == true){
            $(window).scroll(function() {
                if( $(window).scrollTop() + $(window).height() > $(document).height() + 510 ) {
                    if (page < last_page) {
                        page++;
                        var url = "{{URL::route('listing_state')}}?" + formatUrlParams();
                        fetchData(url, '0');
                    }
                }
            });
