...
<div class="topBar" >
    <div class="navbar primary_color navbar-fixed-top megamenu" role="navigation" style="background:#9b59b6">
        <h1 class="section-title-inner" style="text-align:center; padding:20px;">
                <span style="color:white;">Edit State</span>
        </h1>
    </div>
</div>
<div class="clearfix"></div>
<div class="pop_up">
   {!! Form::model($state,['url' => [route('update_state'), $state->id], 'class' => 'form-horizontal','method' => 'post']) !!}
        @if ($errors->any())
    ...
    <div class="row">    
        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
            <div style="display:none;">
                    {!! Form::text('id', $state->id, ['class' => 'form-control']) !!}
                    {!! Form::text('old_slug', $state->slug, ['class' => 'form-control']) !!}
            </div>
            <div>
                <div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}"style=" margin-right: 1px;">
                    {!! Form::label('name', 'State ') !!}
                    {!! Form::label('*', '*', ['class' => 'text-danger']) !!}
                    {!! Form::text('name', $state->name, ['class' => 'form-control','placeholder' => 'State Name']) !!}
                </div>
            </div>
        </div> 
        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
            <div>
                <div class="form-group  {{ $errors->has('country') ? 'has-error' : ''}}"style=" margin-right: 1px;">
                    {!! Form::label('country', 'Country') !!}
                    {!! Form::label('*', '*', ['class' => 'text-danger']) !!}
                    @if($state->country_id=='')
                        {!! Form::select('country', $countries, null, array('class' => 'form-control')) !!}
                    @else
                        {!! Form::select('country', $countries, $state->country_id, array('class' => 'form-control')) !!}
                    @endif
                </div>
            </div>
        </div>
