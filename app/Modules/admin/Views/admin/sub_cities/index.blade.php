...
@section('title')    
    {{Config::get('constants.site.name')}} | Sub Cities
@stop
        ...
        <div>
            <div class="pull-left">
                <h1 class="section-title-inner">
                    <span>Sub Cities</span>
                </h1>
            </div>
            <div class="pull-right"><a class="btn btn-primary btn-small primary_color grouped_elements" href="{{route('add_sub_city')}}">+ Add City</a></div>
            <div class="clearfix"></div>
        </div>
        <hr/>
        <div style="margin-bottom: 20px;">
            <div class="col-md-2" style="padding-left: 0px;">
                <input id="searchinput" name="name" placeholder="Search By Sub City Name" class="form-control input-md" type="search">
            </div>
            <div class="clearfix"></div>
        </div>
        <div class="row userInfo">
            <div class="col-xs-12 col-sm-12">
                <table class="footable footable-loaded tablet breakpoint">
                    ...
                    <tbody id="content">
                        @include('admin::admin.sub_cities.listing', ['sub_city' => $sub_city])
                    </tbody>
                </table>
            </div>
        </div>
        ...
        if(loadingComplete == true){
            $(window).scroll(function() {
                if( $(window).scrollTop() + $(window).height() > $(document).height() + 510 ) {
                    if (page < last_page) {
                        page++;
                        var url = "{{URL::route('listing_sub_city')}}?" + formatUrlParams();
                        fetchData(url, '0');
                    }
                }
            });
