...
<div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12">
        <div>
            ...
            <div class="pull-right" style="margin-left:10px;"><a class="btn btn-primary btn-small primary_color" href="{{route('change_user_password')}}">Change Password</a></div>
            <div class="pull-right"><a class="btn btn-primary btn-small primary_color" href="{{route('add_user')}}">+ Add Admin</a></div>
        ...
        <div class="row userInfo">
            <div class="col-xs-12 col-sm-12">
                <table class="footable footable-loaded tablet breakpoint">
                    ...
                    <tbody id="content">
                        @include('admin::admin.users.listing', ['user' => $user])
                    </tbody>
                </table>
            </div>
        </div>
        ...
        if(loadingComplete == true){
            $(window).scroll(function() {
                if( $(window).scrollTop() + $(window).height() > $(document).height() + 510 ) {
                    if (page < last_page) {
                        page++;
                        var url = "{{URL::route('listing_user')}}?" + formatUrlParams();
                        fetchData(url, '0');
                    }
                }
            });
